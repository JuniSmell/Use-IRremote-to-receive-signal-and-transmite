...
![](https://github.com/JuniSmell/Use-IRremote-to-receive-signal-and-transmite/blob/main/picture/transmit_signal.jpg)
    ...
    接下來打開demo檔，中間的就是所收集的rawData。
    注意有四個功能的數據很特別，存了十組訊號，其他功能只存一組訊號，後面解釋。
    最下面則是負責發送訊號的函數IrSender.sendRaw(名稱、長度、前面提到的9)。
    最後面的9代表的是載波的kHz，大部分的協定(ex,NEC)，其載波為38kHz，這邊就要填38
    發射IR訊號時，預設接腳為Pin 3。
    ** 有四個功能(前進、後腿進、後腿退、立正)的傳輸方式跟其他功能不同，沒辦法直接傳一次訊號就成功，
    我的解法是收集十組數據，然後直接一次傳出去，優點為概念簡單，缺點是靈敏度不高，且容易跟其他功能的訊號混在一起，
    因此每次發射的時間，跟其他功能的訊號發射的時間，其間隔要稍微長一點。
